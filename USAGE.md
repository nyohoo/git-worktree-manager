...
- **正確なマージ状態チェック**: リモートとローカルの状態を詳細に分析

**ブランチ状態の判定:**

`zclean` はブランチを削除する前に、以下を確認します：

1. **✅ リモートにマージ済み**: `origin/master` にマージ済み → 安全に削除可能
2. **🔄 リモートにプッシュ済み**: PR として存在 → 未マージでも安全（GitHub から復元可能）
3. **⚠️ 未プッシュのコミットあり**: ローカルのみのコミット → 削除すると失われる
4. **❌ リモートにプッシュされていません**: 全コミットがローカルのみ → 要注意

**判定ロジック:**
- リモートの `origin/master` との比較でマージ状態を判定
- ローカルの master との比較ではない（よくある誤解）
- PR を push していれば「安全」と判定される
...
### `zprune` - ブランチとworktreeの整理

ブランチと worktree を対話的に整理します。マージ済みブランチやリモートで削除されたブランチを安全に削除できます。
...
**使い方のコツ:**
1. マージ済みブランチを一括削除したい場合 → `Ctrl+A` で全選択
2. 一部だけ残したい場合 → `Ctrl+A` で全選択後、`Tab` で個別に解除
3. 慎重に選びたい場合 → `Tab` で1つずつ選択
4. プレビューで詳細確認 → ブランチにカーソルを合わせると右側に詳細表示

**安全機能:**
- worktree で使用中のブランチは削除候補から自動除外
- 現在のブランチは削除候補から自動除外
- ベースブランチ (main/master) は削除候補から自動除外
- 削除前に確認プロンプトを表示

**検出されるブランチ:**
- ✓ マークのブランチ: main/master にマージ済み
- ⚠ マークのブランチ: リモート追跡なし（リモートで削除済み）
...
5. **定期的に zprune**: マージ済みブランチは定期的に削除してリポジトリを整理
...
Raycast から Git Worktree Manager のタスクを管理できます。
...
2. Script Commands ディレクトリを追加:
   - Raycast を開く（⌘ + Space）
   - "Extensions" → "+" → "Add Script Directory"
   - `~/.config/gwm/raycast` を選択

**利用可能なコマンド:**

- **List Worktree Tasks** - タスク一覧を表示
- **Open Task in Cursor** - タスクを Cursor で開く
- **Open Task in Terminal** - タスクをターミナルで開く
- **Copy Task Path** - タスクパスをクリップボードにコピー

**使い方:**

```
⌘ + Space → "list worktree" → Enter
⌘ + Space → "open cursor" → タスク名入力 → Enter
⌘ + Space → "open terminal" → タスク名入力 → Enter
```

詳細は `~/.config/gwm/raycast/README.md` を参照。
